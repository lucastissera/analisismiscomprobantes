"""Estado de jobs de descarga de «Nuestra Parte» (progreso en pantalla)."""
...
PASOS_NP: tuple[tuple[str, str], ...] = (
    ("login", "Iniciar sesión en ARCA"),
    ("servicio", "Abrir Nuestra Parte"),
    ("tu_informacion", "Entrar a Tu información / ejercicio"),
    ("descargar", "Descargar secciones"),
)
...
def crear_job_np(job_id: str, total: int) -> None:
    with _lock:
        _jobs[job_id] = {"estado": EstadoJobNP(job_id=job_id, total=total, mensaje="Iniciando…")}


def reiniciar_pasos_np(job_id: str) -> None:
    with _lock:
        ...
        st.pasos = [
            ...
            for clave, etiqueta in PASOS_NP
        ]


def callback_paso_np(job_id: str) -> Callable[[str, str], None]:
...
def callback_log_np(job_id: str) -> Callable[[str], None]:
...
def progreso_cuit_np(job_id: str, actual: int, total: int, mensaje: str = "") -> None:
    with _lock:
        item = _jobs.get(job_id)
        if not item:
            return
...
def agregar_archivo_np(job_id: str, download_id: str, ruta: str, nombre: str) -> None:
    with _lock:
        item = _jobs.get(job_id)
        if not item:
            return
...
def marcar_ok_np(job_id: str, *, carpeta: str) -> None:
    with _lock:
        item = _jobs.get(job_id)
        if not item:
            return
...
def marcar_error_np(job_id: str, error: str) -> None:
    with _lock:
        item = _jobs.get(job_id)
        if not item:
            return
...
def marcar_cancelado_np(job_id: str, mensaje: str = "Descarga cancelada.") -> None:
    with _lock:
        item = _jobs.get(job_id)
        if not item:
            return
...
def obtener_job_np(job_id: str) -> dict[str, Any] | None:
    with _lock:
        item = _jobs.get(job_id)
        if not item:
            return None
